...
APPBASE="build/macos-aarch64/Catalyst.app"
...
build() {
    ...
    cp native/build-aarch64/src/Catalyst $APPBASE/Contents/MacOS/
    cp target/Catalyst.jar $APPBASE/Contents/Resources/
    ...
    echo Setting world execute permissions on Catalyst
    pushd $APPBASE
    chmod g+x,o+x Contents/MacOS/Catalyst
    popd

    otool -l $APPBASE/Contents/MacOS/Catalyst
}
...
dmg() {
    ...
    mv Catalyst\ *.dmg Catalyst-aarch64.dmg

    # dump for CI
    hdiutil imageinfo Catalyst-aarch64.dmg

    if ! hdiutil imageinfo Catalyst-aarch64.dmg | grep -q "Format: ULFO" ; then
        echo Format of dmg is not ULFO
        exit 1
    fi

    if ! hdiutil imageinfo Catalyst-aarch64.dmg | grep -q "Apple_HFS" ; then
        echo Filesystem of dmg is not Apple_HFS
        exit 1
    fi

    # Notarize app
    if xcrun notarytool submit Catalyst-aarch64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
        xcrun stapler staple Catalyst-aarch64.dmg
    fi
}
